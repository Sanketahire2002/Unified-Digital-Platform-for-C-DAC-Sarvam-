        ...
        const fetchData = async () => {
            try {
                ...
                const [leaveRequestsResponse, studentsResponse] = await Promise.all([
                    fetch('http://localhost:5000/api/MasterLeaveRequest', {
                        headers: {
                ...
                // Log both data to the console
                console.log('Leave Requests:', leaveRequestsData);
    ...
    const handleAction = async (requestId, action) => {
        ...
        if (confirmed) {
            try {
                ...
                const response = await fetch(`http://localhost:5000/api/MasterLeaveRequest/${action}`, {
                    method: 'POST',
                    headers: {
                ...
                if (response.ok) {
                    ...
                    const fetchData = await fetch('http://localhost:5000/api/MasterLeaveRequest', {
                        headers: {
                                ...
                                <tr key={request.requestId}>
                                    ...
                                    <td style={{ borderRight: '1px solid #dee2e6' }}>
                                        <div className="d-flex flex-column align-items-center">
                                            <span className="fw-bold mb-4">{request.prn}</span> {/* PRN */}
                                            <span className="fw-bold">{getStudentName(request.prn).split(' ')[0]}</span> {/* First Name */}
                                            <span className="fw-bold">{getStudentName(request.prn).split(' ')[1]}</span> {/* Last Name */}
                                        </div>
                                    </td>
