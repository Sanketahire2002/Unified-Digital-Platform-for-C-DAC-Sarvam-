...
const Test = () => {
  ...
  const [timer, setTimer] = useState(5 * 60); // 30 minutes in seconds
  useEffect(() => {
    let interval;
    ...
    if (timer > 0) {
      ...
      // If timer reaches 0, automatically close the window (or end quiz)
      clearInterval(interval);
      // Optional: Call any function to end the quiz or submit answers when time runs out.
      alert("Time's up!");
      window.close();
    }

    return () => clearInterval(interval); // Cleanup the interval on component unmount
  }, [timer]); // Effect will run whenever the timer changes
  ...
  return (
    <div className="container mt-5">
      ...
      <div className="text-center mt-4" style={{
        ...
        {/* Timer Display */}
        <h2 className="mt-5 mb-5 me-3" style={{ color: "red" }}>
