...
import { CodinatorPer } from './CodinatorPer';
    ...
    <div>
      <header className="bg-dark text-white fixed-top p-3 w-100">
        <div className="container-fluid d-flex align-items-center justify-content-between">
          ...
          <div className="d-flex justify-content-center flex-grow-1 align-items-center">
            <Link to="/admin-profile" className="d-flex align-items-center text-decoration-none">
      ...
      <div className={`sidebar bg-dark text-white p-3 vh-100 ${sidebarOpen ? 'sidebar-open' : 'sidebar-closed'}`}>
        ...
        <div className={`sidebar-links ${sidebarOpen ? 'show-links' : ''}`}>
          ...
          <ul className="list-unstyled">
            <li><Link to="/dashboard"><FaHome size={30} />{sidebarOpen && <span className="ms-2">Dashboard</span>}</Link></li>
            <li><Link to="/go-to-admin-dashboard-syllabus"><FaBook size={30} />{sidebarOpen && <span className="ms-2">Syllabus</span>}</Link></li>
            <li><Link to="/admin-attendance"><FaClipboard size={30} />{sidebarOpen && <span className="ms-2">Attendance</span>}</Link></li>
            <li><Link to="/admin-assignment"><FaClipboardList size={30} />{sidebarOpen && <span className="ms-2">Assignment</span>}</Link></li>
            <li><Link to="/admin-materials"><FaClipboardCheck size={30} />{sidebarOpen && <span className="ms-2">Materials</span>}</Link></li>
            <li><Link to="/admin-quiz"><FaRegClipboard size={30} />{sidebarOpen && <span className="ms-2">Quiz</span>}</Link></li>
            <li><Link to="/admin-security"><FaUserLock size={30} />{sidebarOpen && <span className="ms-2">Security</span>}</Link></li>
            <li><Link to="/admin-meetings"><FaList size={30} />{sidebarOpen && <span className="ms-2">Meetings</span>}</Link></li>
            <li><Link to="/admin-feedback"><FaFileAlt size={30} />{sidebarOpen && <span className="ms-2">Feedback</span>}</Link></li>
            <li><Link to="/admin-results"><FaChartBar size={30} />{sidebarOpen && <span className="ms-2">Results</span>}</Link></li>
            <li><Link to="/admin-placements"><FaSuitcase size={30} />{sidebarOpen && <span className="ms-2">Placements</span>}</Link></li>
            <li><Link to="/admin-staff"><FaUsers size={30} />{sidebarOpen && <span className="ms-2">Staff</span>}</Link></li>
            <li><Link to="/admin-users"><FaUserCircle size={30} />{sidebarOpen && <span className="ms-2">Users</span>}</Link></li>
          </ul>
        </div>
      </div>
      ...
      <div className={`main-content p-4 flex-grow-1 ${sidebarOpen ? 'content-expanded' : 'content-collapsed'}`}>
        ...
        <Routes>
          ...
          <Route path="/admin-users" element={<CodinatorPer />} />
          {/* Add other routes */}
        </Routes>
      </div>
    </div>
