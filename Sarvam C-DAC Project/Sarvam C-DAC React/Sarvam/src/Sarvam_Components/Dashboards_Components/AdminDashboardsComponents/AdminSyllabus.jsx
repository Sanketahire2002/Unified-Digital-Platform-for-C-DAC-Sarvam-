...
const AdminSyllabus = () => {
  ...
  const addModule = async () => {
    ...
    try {
      await axios.post(`${API_BASE_URL}/modules`, newModule);
      fetchModules();
      setShowModal(false);
    } catch (error) {
  ...
  const addSubpoint = async () => {
    ...
    try {
      await axios.post(`${API_BASE_URL}/subpoints`, newSubpoint);
      setSubpointName('');
  ...
  const updateSubpoint = async () => {
    ...
    console.log("hii");
    console.log(subpointId);

    try {
      console.log(updatedSubpoint);
      await axios.put(`${API_BASE_URL}/subpoints/${subpointId}`, updatedSubpoint);
      setSubpointName('');
      setSubpointId(null);
      setSelectedModuleId(null);
  ...
  const deleteSubpoint = async (subpointId) => {
    try {
      await axios.delete(`${API_BASE_URL}/subpoints/${subpointId}`);
      fetchModules();
    } catch (error) {
  ...
  const updateModule = async () => {
    ...
    try {
      await axios.put(`${API_BASE_URL}/modules/${selectedModuleId}`, updatedModule);
      fetchModules();
  ...
  return (
    <div className="container mt-5">
      <h2 className="my-4 mt-5">Syllabus</h2>
      ...
      <div className="mb-3">
        ...
        <button className="btn btn-primary mb-3 me-2" onClick={() => setShowModal(true)}>
          Add Module
        </button>
        <button
              ...
              {index === 0 && (
                <>
                  ...
                  {showDeleteModule && (
                    ...
                    >
                      <button
                        className="btn btn-info me-1 mb-2"
                        onClick={() => openUpdateModuleModal(module)}
                      >
                        Update Module
                      </button>
