...
const StudentAttendance = () => {
    ...
    //const { prn } = location.state || { prn: 100000000001 };
    ...
    useEffect(() => {
        const fetchData = async () => {
            if (prn) {
                try {
                    const response = await fetch('http://localhost:8080/api/MasterAttendance/get-by-prn', {
                        method: 'POST',
                        headers: {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const response = await fetch('http://localhost:8080/api/MasterLeaveRequest', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
    ...
    const handleViewRequests = async () => {
        try {
            ...
            const response = await fetch('http://localhost:8080/api/MasterLeaveRequest/GetByPrn', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${token}`, // Add Bearer token to Authorization header
    ...
    const handleWithdraw = async (requestId) => {
        ...
        if (isConfirmed) {
            try {
                ...
                const response = await fetch('http://localhost:8080/api/MasterLeaveRequest/Delete', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'Authorization': `Bearer ${token}`, // Add Bearer token to Authorization header
    ...
    return (
        <div style={{ padding: '20px' }}>
            ...
            {prn ? (
                ...
                <p style={{ fontSize: '24px' }}>No PRN available</p>
